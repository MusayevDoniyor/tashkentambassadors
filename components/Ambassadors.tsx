...
import { MapPin, Send, Linkedin, Instagram, Search, X, Users } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="text-center mb-12">
        ...
        <p className="text-gray-600 max-w-2xl mx-auto font-medium text-lg px-4">
          Toshkent startup ekotizimini rivojlantirishga mas'ul bo'lgan professional jamoa
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
        {filteredAmbassadors.map((amb) => (
          <div key={amb.id} className="bg-white rounded-[2rem] overflow-hidden border border-orange-50 hover:shadow-xl transition-all group flex flex-col h-full animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="p-6 flex flex-col flex-1">
              <h3 className="text-lg font-black text-gray-900 mb-1 tracking-tight leading-none group-hover:text-orange-600 transition-colors uppercase">{amb.name}</h3>
              <p className="text-orange-600 text-[9px] font-black uppercase tracking-[0.15em] mb-4 min-h-[1.5rem]">{amb.role}</p>
              
              <div className="flex items-center space-x-3 mt-auto">
                {amb.socials.telegram && (
                  <a href={`https://t.me/${amb.socials.telegram.replace('@', '')}`} target="_blank" className="w-9 h-9 rounded-lg bg-orange-50 flex items-center justify-center text-orange-600 hover:bg-orange-600 hover:text-white transition-all">
                    <Send size={16} />
                  </a>
                )}
                {amb.socials.linkedin && (
                  <a href={amb.socials.linkedin} target="_blank" className="w-9 h-9 rounded-lg bg-orange-50 flex items-center justify-center text-orange-600 hover:bg-orange-600 hover:text-white transition-all">
                    <Linkedin size={16} />
                  </a>
                )}
              </div>
      ...
      {filteredAmbassadors.length === 0 && (
        <div className="text-center py-20 bg-orange-50/30 rounded-[2.5rem] border-2 border-dashed border-orange-100">
          ...
          <p className="text-gray-400 font-bold uppercase tracking-widest text-xs">Hech qanday a'zo topilmadi</p>
          <button 
            onClick={() => setSearchQuery('')}
