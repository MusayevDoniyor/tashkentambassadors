...
const Events: React.FC = () => {
  ...
  const formatPhone = (val: string) => {
    ...
    // If it doesn't start with 998, prepend it if it's long enough
    let formatted = x;
    ...
    // I'll use +998 XX XXX XX XX for simplicity and readability
    let res = "";
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    // Basic validation: must be at least +998 XX XXX XX XX (17 chars with spaces)
    ...
    try {
      // Check duplicate using the exact same format
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300">
            <div className="p-8 md:p-12">
              <button
                onClick={() => setShowRegModal(false)}
                className="absolute top-6 right-6 p-2 hover:bg-gray-50 rounded-xl text-gray-400"
              >
                <X size={20} />
              </button>
              ...
              {regStatus === "success" ? (
                <div className="py-12 text-center">
                  ...
                  <h4 className="text-2xl font-black mb-2">Muvaffaqiyatli!</h4>
